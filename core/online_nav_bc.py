...
class OnlineNavigatorBC:
    ...
    def run_one_episode(self, episode_num):
        ...
        print(f"\n{'='*50}")
        print(f"Starting BC Policy Validation Episode {episode_num}")
        print(f"{'='*50}\n")
        ...
        try:
            while not self.mission_complete:
                ...
                if current_action_index >= len(action_buffer):
                    # Use BC policy to predict complete action sequence
                    with torch.no_grad():
                        action_sequence = self.nav_policy(image_sequence).squeeze(0).cpu().numpy()  # [T, Da]
                ...
                print(f"Frame {frame_id}: Command = {command:.2f}° (Action {current_action_index}/{len(action_buffer)})")
